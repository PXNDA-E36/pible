use std::{collections::{BTreeMap, BTreeSet}, fs, str::FromStr, time::Duration};
...
pub(crate) async fn main() -> bluer::Result<()> {
    ...
    let ciphertext = cipher.encrypt(nonce, asset_id_hash.as_bytes())?;
    ...
    const SERVICE_UUID: Uuid = Uuid::from_str("c193b907-6f78-4769-aafa-83e807c9c0a6").unwrap();
